...
sPLNdiag_model <- function(Y = Y, X = X, formuleSeg = ~bs(j,10) , formPLN = "~factor(hr)", 
  ...
  all_tries = FALSE
  i = 0
  print(paste0("Models with ",as.character(S), " segments"))
  while((all_tries == FALSE) & i < max_try){
    ...
    while(hotRestart != FALSE){
      ...
      while(fin_iter != TRUE & iter < itermax & skip_try != TRUE){
        ...
        ## UPDATE THE MIXTURE MODEL VIA OPTIMIZATION OF PLN
  ...
  n = dim(Y)[2]
  nu = (S-1)*dim(Xms)[2] + S * dim(Xmm_PLN)[2] *n  + S * n
