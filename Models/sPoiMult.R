...
sPoiMult_model <- function(Y = Y, X = X, formuleSeg = ~bs(j,10) , formulepois =  ~bs(hr,5), formuleMult =   ~bs(hr,5),
  ...
  while((all_tries == FALSE) & i < max_try){
    ...
    #The initialization of the parameters and the EM algo are integrated in a while loop which allows a hotRestart 
    #when a segment has disappeared
    ...
    while(hotRestart != FALSE){
      ...
      if(iter > 0){
        ...
        #segments, alpha parameters for smoothed model
        tryCatch(
        ...
        if((sum(max_cons <= min_seg) > 0)){
          ...
          while((sum(max_cons <= min_seg) > 0)){
            ...
            for(seg_disp in list_seg_disp){
              c = c + 1
              print(paste0("Segment", as.character(seg_disp), " disappear, a new one is built"))
  ...
  n = dim(Y)[2]
  nu =  (S-1)*dim(Xms)[2] + S * (dim(Xmp)[2]+1) + S * dim(Xmm)[2] * (n-1)
