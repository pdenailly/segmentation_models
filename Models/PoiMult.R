...
PoiMult_model <- function(Y = Y, X = X, formuleSeg = ~bs(j,10) , formulepois =  ~bs(hr,5), formuleMult =   ~bs(hr,5),
  ...
  while((all_tries == FALSE) & i < max_try){
    ...
    Xmp = model.matrix(as.formula(formulepois),X)
    ...
    parameters = list(poisson=list(),multinomial=list())
    ...
    for(s in seq(1,S)){
      ...
      #Poisson
      tryCatch(
        parameters$poisson[[s]] <- suppressWarnings(glm.fit(y=Ytot,x=Xmp,weights = vec_poids,family = poisson(),control=list(epsilon = 1e-2,maxit=10,trace=F))),
        error = function(e){
          message("one segment seems to have disappeared")
          #message(e)
          skip_try <<- TRUE
        })
      if(skip_try) { next }
      ...
      #Multinomial
      tryCatch(
        parameters$multinomial[[s]] <- multinom(Y ~ Xmm,weights = vec_poids,trace=F,maxit=70,MaxNWts = 10000000),
        error = function(e){
          message(e)
          skip_try <<- TRUE
        })
    ...
    #First segment initialisation
    Tis_j = matrix(0,nbJ,S)
    lTis_j = matrix(0,nbJ,S)
    
    for(s in seq(1,S)){
      theta = parameters$multinomial[[s]]$fitted.values
      lTis_j[,s]= as.matrix(as.data.frame(cbind(dpois(Ytot,parameters$poisson[[s]]$fitted.values,log = TRUE) +
                                                  sapply(1:nbT, function(i) dmultinom(Y[i,], prob = theta[i,],log=TRUE)), X$j))  %>% 
                              `colnames<-`(c("ll","j")) %>% 
    ...
    #The initialization of the parameters and the EM algo are integrated 
    #in a while loop which allows a hotRestart when a segment has disappeared
    ...
    while(hotRestart != FALSE){
      ...
      if(iter > 0){
        ...
        #Formatting in probabilities of belonging to each segment by time slot
        ...
        for(s in seq(1,S)){
          #poisson
          tryCatch(
            parameters$poisson[[s]] <- glm.fit(y=Ytot,x=Xmp,weights = as.matrix(Tis_t[,s]),family = poisson(), control=list(epsilon = 1e-2,maxit=10,trace=F)),
            error = function(e){
              message("Warning: a segment disapeared")
              #message(e)
              skip_try <<- TRUE
            })
          if(skip_try) { next }
          ...
          #Multinomial
          tryCatch(
            parameters$multinomial[[s]] <- multinom(Y ~ Xmm,weights = as.matrix(Tis_t[,s]),trace=F,maxit=50,MaxNWts = 10000000),
            error = function(e){
              message(e)
              skip_try <<- TRUE
            })
      ...
      while(fin_iter != TRUE & iter < itermax & skip_try != TRUE){
        ...
        if((sum(max_cons <= min_seg) > 0)){
          ...
          while((sum(max_cons <= min_seg) > 0)){
            ...
            for(seg_disp in list_seg_disp){
              c = c + 1
              print(paste0("Segment ", as.character(seg_disp), " disapear, creation of a new one"))
              ...
              #Tis_j[,seg_disp] = rep(0,nrow(Tis_j))
          ...
          hotRestart = TRUE
          print("Hot Restart")
          if(hotRestart){break} #back to parameter initialization
          if(nbr_supr > 5){
            hotRestart = FALSE
            skip_try = TRUE
            next
        ...
        #Poisson and Multinomiale parameters estimation
        for(s in seq(1,S)){
          coefs_pois = parameters$poisson[[s]]$coefficients
          coefs_pois[is.na(coefs_pois)] = 0
          
          #Poisson
          tryCatch(
            parameters$poisson[[s]] <- glm.fit(y=Ytot,x=Xmp,weights = as.matrix(Tis_t[,s]),family = poisson(), start = coefs_pois,control=list(epsilon = 1e-5, maxit=20,trace=F)),
            error = function(e){
              message(e)
              skip_try <<- TRUE
            })
          ...
          #Multinomial
          tryCatch(
            parameters$multinomial[[s]] <- multinom(Y ~ Xmm,weights = Tis_t[,s]+1e-16,Wts=parameters$multinomial[[s]]$wts,maxit=50,trace=F,MaxNWts = 10000000),
            error = function(e){
              message(e)
              skip_try <<- TRUE
            })
  ...
  n = dim(Y)[2]
  nu =  S * (dim(Xmp)[2]+1) + S * dim(Xmm)[2] * (n-1)
