...
NegPol_model <- function(Y = Y, X = X, formuleSeg = ~bs(j,10) , formulenb =  ~bs(hr,5), formuleMult =   ~bs(hr,5),
  ...
  while((all_tries == FALSE) & i < max_try){
    ...
    #The initialization of the parameters and the EM algo are integrated in a while loop which allows 
    #a hotRestart when a segment has disappeared
    ...
    while(hotRestart != FALSE){
      ...
      if(iter > 0){
        ...
        #pi values: weights of belonging of days to segments
      ...
      while(fin_iter != TRUE & iter < itermax & skip_try != TRUE){
        ...
        if((sum(max_cons <= min_seg) > 0)){
          ...
          hotRestart = TRUE
          print("Hot Restart")
          if(hotRestart){break} #Back to parameters initilization
          if(nbr_supr > 5){
            hotRestart = FALSE
            skip_try = TRUE
            next
        ...
        #Negative binomial and Multinomiale Dirichlet multinomial parameters 
        for(s in seq(1,S)){
          Tis_t[,s][Tis_t[,s]==0] = 0.1
