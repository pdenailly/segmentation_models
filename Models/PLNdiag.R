PLNdiag_model <- function(Y = Y, X = X, formuleSeg = ~bs(j,10) , formPLN = "~factor(hr)-1", 
  ...
  while((all_tries == FALSE) & i < max_try){
    ...
    for(s in seq(1,S)){
      ...
      tryCatch(
        parameters$pln[[s]] <- suppressWarnings(PLN(as.formula(paste("Abundance",as.character(formPLN))),data_Count, weights =  vec_poids, control = list(covariance = "diagonal",maxeval=500,trace=0))),
        error = function(e){
          message(e)
          skip_try <<- TRUE
        })
    ...
    while(hotRestart != FALSE){
      ...
      if(iter > 0){
        ...
        #pi : weights of days belonging to segments
        pijs = sapply(as.vector(colSums(Tis_j)/nbJ), function(i) rep(i,nbJ))
        
        
        # Formatting in probas of belonging to each segment by time slots
        ...
        for(s in seq(1,S)){
          ...
          tryCatch(
            parameters$pln[[s]] <- suppressWarnings(PLN(as.formula(paste("Abundance",as.character(formPLN))),data_Count, weights =  Tis_t[,s] , control = list( covariance = "diagonal",maxeval=500,trace=0))),
            error = function(e){
              message(e)
              skip_try <<- TRUE
            })
      ...
      while(fin_iter != TRUE & iter < itermax & skip_try != TRUE){
        ...
        ## UPDATE THE MIXTURE MODEL VIA OPTIMIZATION OF PLN model
        ...
        for(s in seq(1,S)){
          ...
          tryCatch(
            parameters$pln[[s]] <- suppressWarnings(PLN(as.formula(paste("Abundance",as.character(formPLN))),data_Count, weights = Tis_t[,s] , 
                                                        control = list(inception = parameters$pln[[s]], covariance = "diagonal",maxeval=500,trace=0))),
            error = function(e){
              message(e)
              skip_try <<- TRUE
            })
